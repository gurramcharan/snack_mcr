...
export const HomePage = () => {
    ...
    return (
        <div className='main-container'>
            <input className='search' type="text" name="search" id="search" placeholder='Search with Products or Ingredients...' onChange={(e) => setSearchData(e.target.value)} />
            <table className='table-container'>
                <tr className='table-fields'>
                    {headings.map((head) => (
                        <th className='table-fields'><button className='heading-btn' onClick={() => handleBtnClick(head)}>{head}</button></th>
                    ))}
                </tr>
